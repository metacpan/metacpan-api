...
my $dist   = $mcpan->release('MetaCPAN-Client');
```
...
```sh
curl -XPOST fastapi.metacpan.org/v1/release/_search -d '{
  "query": {
    "match_all": {}
  },
  "size": 5000,
...
```sh
curl -XPOST fastapi.metacpan.org/v1/file/_search -d '{
  "query": { "match_all": {} },
  "facets": {
    "size": {
      "statistical": {
...
```sh
curl -XPOST fastapi.metacpan.org/v1/release/_search?size=100 -d '{
  "query": {
    "match_all": {},
    "range" : {
        "release.date" : {
...
```sh
curl -XPOST fastapi.metacpan.org/v1/author/_search -d '{
  "query": {
    "match_all": {}
  },
  "filter": {
...
```sh
curl -XPOST fastapi.metacpan.org/v1/favorite/_search -d '{
  "query": { "match_all": {}
   },
  "facets": {
    "leaderboard": {
...
```sh
curl -XPOST fastapi.metacpan.org/v1/distribution/_search -d '{
  "query": {
    "match_all": {}
  },
  "size": 1000,
